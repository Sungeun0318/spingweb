...
public class FileService {
    ...
    public String upload(MultipartFile uploadFile) {
        ...
        if (uploadFile.isEmpty()) {
            return null; // 업로드 실패 : 파일 없어서
        }

        // 2) 업로드할 파일의 경로 *서버경로*, 개발자(src파일) --배포/실행--> 서버(build파일) <--- 클라이언트(사용자)
        File uploadPath = new File(uploadDir); // 업로드할 uploadDir + 파일명을 file 객체내 대입
        ...
        // 3) 업로드, 실제업로드경로 + 파일명
            // *** 만약에 서로 다른 사람/요청이 동일한 파일명으로 다른 파일 업로드하면 고유 식별 깨짐 ***
            // 즉] 파일명은 동일하지만 다른 파일일 수 있다. UUID vs 날짜/시간(밀리초) vs PK번호 조합
        String uuid = UUID.randomUUID().toString(); // UUID란? 중복없는 난수 문자열 생성 함수(고유성 보장)
            // * UUID_파일명 : UUID와 파일명 사이에 _언더바 구분하자(왜? 추후에 분리 해야 하니까)
        ...
        try {
            ...
            return uploadRealPath.getAbsolutePath();
        }catch (IOException e){
