...
/*
    - 영속성이란? 자바 객체와 데이터베이스 데이터간 매핑/연결 상태
        - 영속성 해제란? 자바객체와 데이터베이스 데이터 간 매핑/연결 해제
    - cascade 속성이란? PK와 FK 제약조건 옵션
        CascadeType.ALL : 부모가 삭제/수정/저장되면 자식도 같이 삭제/수정/저장 반영된다.
    ...
    - fetch 부모[PK] 조회시 자식[FK] 관계에서 엔티티 조회여부 선택
        FetchType.EAGER : (기본값)해당 엔티티 조회시 참조 엔티티 모두 죽시 조회한다.
            - 특징 : 초기 로딩 느리다. 재사용시 빠르다. 불필요한 정보까지 있을 경우 성능저하
        FetchType.LAZY : 해당 엔티티 조회시 참조 에닡티는 조회하지 않는다. <참조 엔티티 호출시 조회>
    ...
    - 단방향/양방향 활용
        ...
        - 만약에 3번 게시물에 댓글 등록 한다면, ** fk 필드에 fk값인 3 대신에 3갖는 fk엔티티 찾아서 대입하자
