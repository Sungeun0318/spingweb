...
@RestController // + ResponseBoby 포함 = 사용처 : 값(JSON) 반환
@RequestMapping("/day02") // @RequestMapping("/공통URL") // 해당 컨트롤러내 메소드들이 사용하는 공통URL 정의한다.
public class RestController3 {
    // 1. 클래스가 @RestController 이므로 @RespnseBody 생략해도 된다.
    ...
    @DeleteMapping("/task9") // GET/DELETE 메소드는 구조와사용법이 동일하다.
    public int method4( String name , @RequestParam( defaultValue = "19") int age ){
        // 만약에 @RequestParam 생략해도 매개변수 매핑(연결) 가능하다.
        // 만약에 쿼리스트링에 매개변수명이 존재하지 않을때 기본값으로 설정하기 , @RequestParam( defaultValue="초기값" )
    ...
    public int method5(@RequestParam Map<String,Object> map){ // 여러개 매개변수를 하나의 Map 타입으로 받을 수 있다.
        System.out.println("RestController3.method5");
        System.out.println("map = " + map); // {name=유재석, age=40}
        return 10;
    }
