...
public class JWTService {
    ...
    private final String 비밀번호 = "12345678912345678912345678912345";
    ...
    public String 토큰생성(String data){
        String token = Jwts.builder() // 토큰 생성 시작
                ...
                .claim("data", data)
                .setIssuedAt(new Date()) // 토큰 발급날짜/시간, new Date() : 시스템날짜/시간 반환
                // .setExpiration(new Date(System.currentTimeMillis() + 1000 * 초단위 ))
                .setExpiration(new Date(System.currentTimeMillis() + 1000 * 20)) // 토큰 유지 / 유효시간
