...
class SuperClass{ // 상위클래스
    void method(){}
}
class SubClass extends SuperClass{ // 하위클래스
    @Override void method(){} // 상위 클래스로부터 물려받은 메소드 재정의한다.
...
public class Exam1 {
    public static void main(String[] args) {
        ...
        /*
            @ 어노테이션
            1. 정의 : 자바에서 코드의 메타데이터를 추가하는 문법 *메타데이터란? 특정한 데이터를 설정하는 데이터
            2. 목적 : 1. 메타데이터 제공 2. 코드 간소화 3. 가독성 향상
            3. 종류
                1. 표준 어노테이션 : 자바에서 기본적으로 제공하는 어노테이션
                    @Override : 상위 클래스로부터 메소드를 재정의
                2. 메타 어노테이션 : 코드를 정의하거나 동작 제어할 때 사용되는 어노테이션
                    스프링 프레임워크
            ...
            4. 어노테이션 만들기
                ...
                2. 인터페이스 위에 유지기간, 실행위치 설정
                    1] @Retention(RetentionPolicy.RUNTIME) : 프로그램 실행중 유지 뜻
                       ...
                       @Retention(RetentionPolicy.SOURCE) : 컴파일(코드 -> 번역) 이후까지만 유지 뜻
            ...
            5. 스프링 어노테이션
                ...
                * 인터페이스란? 사용자와 시스템 간의 중개

            6. 동적 로딩이란? 리플렉션, 실행 중에 클래스를 읽어와서 객체 생성과정
