...
// === 방법 3 : 다른 클래스의 메소드를 호출하는 방법 ===
class TaskDao3{
    private TaskDao3(){}
    private static TaskDao3 instance = new TaskDao3();
    public static TaskDao3 getInstance(){
        return instance;
    }
    void method1(){}
}
...
class TaskController4{
    // DI 방법1
    // @Autowired private TaskDao4 taskDao4; // 해당하는 변수에 스프링 빈에 등록된 객체를 주입 해준다.
    // DI 방법2 *권장*
...
/*
    IOC : 제어의 역전, 개발자가 직접 해야하는 객체 제어권을 스프링에게 위임
        ...
        2. 스프링에서 스프링 컨테이너 *컨테이너* 담당한다.
        3. 목적 : 개발자에게 편의성, 규칙성(협업간의 객체충돌)에 따른 관리
        4. 주요어노테이션
    ...
    DI : 의존성 주입, IOC 표현하기위한 기술 중 하나, 위임한 객체 제어권에서 객체 가져오기.
        1. 정의 : 객체를 직접 생성하지 않고 외부(스프링 컨테이너)로부터 객체 주입받도록 하는 방법
