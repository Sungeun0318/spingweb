...
public class Exam3 {
    public static void main(String[] args) {
        ...
        // [1] 리스트 변수명.stream().forEach(); ,중간연산 없이 바로 최종출력
        ...
        number.stream()
                .map(x -> {return x * 2;})    // 중간연산
                .forEach( (중간연산결과) -> {System.out.println("result = " + result);}); // 중간연산 결과를 출력한다.

        // [4] 리스트변수명.stream().map(중간연산).forEach(최종출력);
        number.stream() // 리스트내 데이터들의 흐름 시작
                .filter(x -> x % 2 == 0) // 중간연산, 짝수찾기 반환한다.
                .forEach(y -> System.out.println("y = " + y)); // 최종출력 :

        // [5] 리스트변수명.stream().map(중간연산).forEach(최종출력);
        number.stream() // 리스트내 데이터들의 흐름 시작
