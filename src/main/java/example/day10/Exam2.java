...
public class Exam2 {
    ...
    public static void main(String[] args) {
        ...
        // [2] 익명구현체, 구현체? 추상메소드를 구현한 객체, 익명구현체란? 클래스없이 구현한 인스턴스
        ...
        int result2 = calculator.plus(3,2); // 1
        System.out.println("result2 = " + result2);
        // [3] 람다 표현식, -> (매개변수) -> {구현부}
        Calculator calculator2 = ((x, y) ->  x-y); // ???
        int result3 = calculator2.plus(3,2);
        System.out.println("result3 = " + result3);

        // [4] 람다 표현식을 사용하는 *함수형* 인터페이스들
        // < > : 제네릭이란? 인스턴스(객체) 생성할 때 인스턴스(객체)내 멤버들의 타입 정의한다.
        // List<Integer> : List 객체 생성시 내부에 Integer 타입으로 항목 구성하겠다 뜻
        ...
        // [4-2] Supplier<T> T : 입력없이 결과 반환 값 타입, R : X
        Supplier<Double> supplier = () -> 3.14;  // return 생략시 {} 같이 생략
        System.out.println("supplier.get() = " + supplier.get());

        // [4-3] Consumer<T> T : 입력받은 값 타입이고 결과 값 없다. , accept(T) 메소드
