...
public class MemberController2 {
    ...
    public ResponseEntity<?> login(@RequestBody MemberDto loginDto) {
        ...
        // 1] 만약에 로그인 성공이면 세션 부여
        if(result){
            String token = jwtService.createToken(loginDto.getMid()); // 2] 로그인 성공한 정보(아이디)
            return ResponseEntity.ok() // 3] 토큰(jwt)은 주로 세션과 다르게 서버에 저장하지 않고 클라이언트에 저장한다.
    ...
    public ResponseEntity<?> myInfo(@RequestHeader("Authorization") String token) {
        ...
        // 1] @RequestHeader("Authorization") String toke 매개변수로 받는다.
        // 2] 만약에 헤더가 없거나 토큰이 없으면 비로그인
        if(token == null || !token.startsWith("Bearer ")){
            return ResponseEntity.ok(false);
        }
        // 3] 토큰 추출 , 문자열.replace("기존문자", "새로운문자")
        token = token.replace("Bearer ", ""); // Bearer 없애기, 띄어쓰기 주위
        // Bearer 토큰값 -- >  토큰값
        // 4] 토큰에서 값(클레임) 추출
        ...
        // 5] 토큰에서 꺼낸 값(mid)으로 회원정보 요청하기 반환하기
