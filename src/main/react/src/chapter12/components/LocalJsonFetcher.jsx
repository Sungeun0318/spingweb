...
const GlobalTop = (props) => {
    ...
    const axios1 = async () => { // axios 통신하는 함수 하나 만든다. async 동기화한다.
        const response = await axios.get('./json/myData.json'); // await 동기화된 axios 통신한다.
        // response 에는 HTTP 응답 관련 정보들이 객체로 들어있다.
        // 그중에 response.data는 실질적인 결과물(내용물/body) 있다.
        const result = response.data; // 통신 결과내 .data가 실질적인 결과물(내용물/boby) 가져온다.
    ...
    // (2) useEffect (() => {}, [])                : 최초실행
    // (3) useEffect (() => {}, [상태변수1, 상태변수2]) : 최초실행, 특정 상태변경 될 때 마다 실행
    ...
    // 리스트/배열변수명.map((반복변수) => {retrun(<> JSX</>)}) // 주로 HTML 구성할 때 사용한다.
